...
enum UnlimitedOCRModelManifest {
    ...
    static let totalBytes = artifacts.reduce(Int64(0)) { partialResult, artifact in
        partialResult + artifact.size
    }

    static func downloadURL(for artifact: UnlimitedOCRModelArtifact) -> URL? {
        URL(
            string: "https://huggingface.co/\(repository)/resolve/\(revision)/\(artifact.path)?download=true"
        )
    }
}
